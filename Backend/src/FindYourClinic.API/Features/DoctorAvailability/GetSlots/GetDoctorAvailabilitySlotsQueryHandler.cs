...
public class GetDoctorAvailabilitySlotsQueryHandler : IRequestHandler<GetDoctorAvailabilitySlotsQuery, ApiResponse<List<DateTime>>>
{
    ...
    public async Task<ApiResponse<List<DateTime>>> Handle(GetDoctorAvailabilitySlotsQuery request, CancellationToken cancellationToken)
    {
        ...
        var slots = await BuildAvailableSlotsAsync(doctorProfileId, request.Date, cancellationToken);
        return ApiResponse<List<DateTime>>.Ok(slots);
    }

    private async Task<List<DateTime>> BuildAvailableSlotsAsync(Guid doctorProfileId, DateOnly date, CancellationToken cancellationToken)
    {
        ...
        var windows = await _dbContext.DoctorAvailabilities
            .AsNoTracking()
            .Where(x => x.DoctorProfileId == doctorProfileId && x.IsActive && x.DayOfWeek == date.DayOfWeek)
        ...
        foreach (var window in windows)
        {
            ...
            {
                var dateTime = date.ToDateTime(TimeOnly.FromTimeSpan(current), DateTimeKind.Utc);
                if (dateTime > DateTime.UtcNow && !bookedSet.Contains(dateTime))
                {
                    slots.Add(dateTime);
                }
